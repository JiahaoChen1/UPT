...
class VPT(nn.Module):
    def __init__(self, vpt_len, seq_len, patch_size, emb_dim, dtype=None, num_l=0):
        super().__init__()
        ...
        # if num_l == 11:
        #     self.prompt_add1 = nn.Parameter(torch.empty(vpt_len, emb_dim, dtype=dtype))
        #     nn.init.uniform_(self.prompt_add1, -init_val, init_val)

        #     self.prompt_add2 = nn.Parameter(torch.empty(vpt_len, emb_dim, dtype=dtype))
        #     nn.init.uniform_(self.prompt_add2, -init_val, init_val)

    @property
    def dtype(self):
        return self.prompt.dtype

    def forward(self, x, num_block=-1):
        x = x[:, :self.seq_len, :]
        prompt = self.prompt.expand(x.shape[0], -1, -1)
        # if num_block == 11:
        #     prompt_add1, prompt_add2 = self.prompt_add1.expand(x.shape[0], -1, -1), self.prompt_add2.expand(x.shape[0], -1, -1)
        #     out1, out2, out3 = torch.cat([x, prompt], dim=1), torch.cat([x, prompt_add1], dim=1), torch.cat([x, prompt_add2], dim=1)
        #     # print(out1.shape, out2.shape, out3.shape)
        #     return torch.cat([out1, out2, out3], dim=0)
